...
export default function VirtualFitting({ theme, setTheme }) {
  ...
  useEffect(() => {
    ...
    if (items.length === 0) {
      ...
      toast.addToast({
        type: "warning",
        title: "尚未選擇衣物",
        message: "請先在衣櫥中選擇要試穿的單品",
      });

      // 為了避免跟 toast 更新打架，稍微延遲導頁
      setTimeout(() => navigate("/wardrobe"), 50);
      return;
    }
    ...
    items.forEach((item) => {
      ...
      if (category === "帽子") positions.hat = item;
        ...
        positions.top = item;
      else if (category === "褲子" || category === "裙子")
        positions.bottom = item;
  ...
  const handlePhotoUpload = (e) => {
    ...
    reader.onloadend = () => {
      ...
      hasAutoGeneratedRef.current = true;
      // 上傳照片後自動重新生成
      autoGenerateImage(selectedItems, base64);
    };
