...
export default function Assistant({ theme, setTheme }) {
  const [messages, setMessages] = useState([
    { id: 1, role: 'assistant', text: '嗨！我是你的穿搭小助手，有什麼穿搭建議都歡迎詢問我喔' },
  ]);
  ...
  function computeCardHeightPx() {
    ...
    const headerH = headerEl ? headerEl.getBoundingClientRect().height : (4 * rootFontSize); // fallback 4rem

    // 手機額外空間：10em -> px
    const mobileExtraPx = 10 * rootFontSize;

    const vh = window.innerHeight;
    const final = isMobile ? Math.max(200, Math.floor(vh - mobileExtraPx)) : Math.max(200, Math.floor(vh - headerH));
    return final;
  }
      ...
      <div className="max-w-5xl mx-auto mt-4">
        ...
        >
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4" aria-live="polite">
            {messages.map((m) => (
              <div key={m.id} className={`mb-4 flex ${m.role === 'assistant' ? 'items-start' : 'justify-end'}`}>
                {m.role === 'assistant' && (
                ...
                <div>
                  <div className={`px-4 py-2 max-w-[70ch] break-words ${m.role === 'assistant' ? 'bg-gray-100 text-gray-800 rounded-xl rounded-tl-none' : 'bg-indigo-600 text-white rounded-xl rounded-tr-none'}`}>
                    {m.text}
                  </div>
                </div>
                ...
                {m.role === 'user' && (
                  <div className="flex-shrink-0 ml-3">
                    <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-700 font-medium">U</div>
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="border-t px-4 py-4 bg-white flex-shrink-0">
            <div className="flex gap-2 mb-3 flex-wrap">
              {quickPrompts.map((q) => (
                <button
                  key={q}
                  onClick={() => { addMessage('user', q); simulateBackendReply(q); }}
            ...
            <form onSubmit={handleSend} className="flex items-center gap-3">
              ...
              />
              <button type="submit" className="bg-indigo-600 text-white px-4 py-3 rounded-xl" disabled={sending}>
                {sending ? '傳送中…' : '送出'}
              </button>
            </form>
          </div>
...
function Dot({ delay = 0 }) {
  return <span className="dot" style={{ animationDelay: `${delay}ms` }} aria-hidden="true" />;
}
